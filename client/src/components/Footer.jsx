      ...
      <div className="pb-8 xl:grid xl:grid-cols-5 xl:gap-8">
        <div className="grid grid-cols-2 gap-8 xl:col-span-4">
          <div className="md:grid md:grid-cols-2 md:gap-8">
            <div>
              <h3 className="text-base font-medium text-white">Fashios</h3>
              <ul role="list" className="mt-4 space-y-4">
                {navigation.fashios.map((item) => (
                  <li key={item.name}>
            ...
            <div className="mt-12 md:mt-0">
              <h3 className="text-base font-medium text-white">For Him</h3>
              <ul role="list" className="mt-4 space-y-4">
                {navigation.forHim.map((item) => (
                  <li key={item.name}>
                    <a href={item.href} className="text-base text-gray-300 hover:text-white">
                      {item.name}
                    </a>
          ...
          <div className="md:grid md:grid-cols-2 md:gap-8">
            <div>
              <h3 className="text-base font-medium text-white">For Her</h3>
              <ul role="list" className="mt-4 space-y-4">
                {navigation.forHer.map((item) => (
                  <li key={item.name}>
                    <a href={item.href} className="text-base text-gray-300 hover:text-white">
                      {item.name}
                    </a>
            ...
            <div className="mt-12 md:mt-0">
              <h3 className="text-base font-medium text-white">Devices</h3>
              <ul role="list" className="mt-4 space-y-4">
                {navigation.devices.map((item) => (
                  <li key={item.name}>
                    <a href={item.href} className="text-base text-gray-300 hover:text-white">
                      {item.name}
                    </a>
        ...
        <h1 className="text-4xl text-white font-bold font-handwrite">Lucky.</h1>
      ...
      <div className="mt-8 border-t border-gray-700 pt-8 md:flex md:items-center md:justify-between">
        <div className="flex space-x-6 md:order-2">
          {navigation.social.map((item) => (
            <a key={            
              item.name} href={item.href} className="text-gray-400 hover:text-gray-300">
        ...
        <p className="mt-8 text-base text-gray-400 md:order-1 md:mt-0">
          &copy; 2023 Pouriya, Inc. All rights reserved.
        </p>
      </div>
    </div>
